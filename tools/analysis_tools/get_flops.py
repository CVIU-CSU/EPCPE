...
def inference(args, logger):
    ...
    model_decoder_params = sum(p.numel() for n,p in model.named_parameters() if 'decoder' in n)
    model_no_backbone_params = sum(p.numel() for n,p in model.named_parameters() if 'backbone' not in n)
    model_tunable_params = sum(p.numel() for n,p in model.named_parameters() if 'backbone' not in n or 'S_Adapter' in n)
    model_bbox_head_params = sum(p.numel() for n,p in model.named_parameters() if 'bbox_head' in n )
    
    print('model_total_params:' + str(model_total_params))
    print('model_tunable_params:' + str(model_tunable_params))
    print('model_no_backbone_params:' + str(model_no_backbone_params))
    print('model_decoder_params:' + str(model_decoder_params))
    print('model_bbox_head_params:' + str(model_bbox_head_params))
