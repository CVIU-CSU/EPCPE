...
from ..functional import compute_degree_cm_mAP
...
class CocoMetricCPPF(BaseMetric):
    ...
    def __init__(self,
                 ...
                 dataset_name: str = 'nocs',
                 full_rot: Optional[str] = None,
                 synset_names: List = ['BG','bottle', 'bowl', 'camera', 'can', 'laptop', 'mug']) -> None:
        ...
        assert self.dataset_name in ['nocs','sunrgbd'], 'dataset_name should be nocs or sunrgbd'
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            result['rots'] = pred['rots'].cpu().numpy()
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        logger.info("TEST computing metrics on CPPF now")
        ...
        for metric in self.metrics:
            ...
            if metric == 'pose':
                ...
                #synset_names=['BG','bottle', 'bowl', 'camera', 'can', 'laptop', 'mug'] #nocs
                #synset_names=['BG', 'bed','table','sofa','chair','toilet','desk','dresser','night_stand','bookshelf','bathtub'] #sunrgbd
                #synset_names=['BG', 'bed','table','sofa','chair','bookshelf','bathtub'] #cppf
                if self.synset_names==None:
                    synset_names=['BG','bottle', 'bowl', 'camera', 'can', 'laptop', 'mug']
                else:
                    synset_names=self.synset_names
                ...
                try: 
                    ...
                    #print(cfg.work_dir)
                    pred_results_base_dir = osp.join(cfg.work_dir,'pred_results/cppf/')
                except:
                    pred_results_base_dir='pred_results/cppf/eval'
                #print("pred_results_base_dir",pred_results_base_dir)
                #pred_results_base_dir='pred_results/cppf/spilts_xyz/'
                ...
                for i,result in zip(range(len(self.img_ids)),preds):
                    ...
                    #np.array other list
                    gt_label=np.array([class_id+1 for class_id in gt_label],np.int32)
                    ...
                    pred_label=np.array([class_id+1 for class_id in pred_label],dtype=np.int32)
                    ...
                    if self.dataset_name=='nocs':
                        gt_up_syms=np.zeros_like(gt_label,dtype=bool)
                        for i, (cls_id, vis) in enumerate(zip(gt_label, gt_handle_visibility)):
                        ...
                        if self.full_rot:
                            ...
                            for i, (cls_id, vis) in enumerate(zip(gt_label, gt_handle_visibility)):
                                if vis == 0:  # handle not seen, assume up summetry
                                    ...
                                    gt_up_syms[i] = True
                        if not self.full_rot:
                            gt_up_syms=np.ones_like(gt_label,dtype=bool)
                    ...
                    pred_result['gt_up_syms']=gt_up_syms
                ...
                #comupte mAP
                evaluate_cppf(pred_results,pred_results_dir,synset_names,logger)
...
def evaluate_cppf(pred_results=None,pred_results_dir=None,synset_names=None,logger=None):
    degree_thres_list = list(range(0, 61, 1))+[360]
    shift_thres_list = [i / 2 for i in range(21)]+[100]
    iou_thres_list = [i / 100 for i in range(101)]

    degree_thres_list = list(range(5, 21, 5)) + [40,60,360]
    shift_thres_list = list(range(5, 31, 5)) + [100]
    #typename=['bed','table','sofa','chair','toilet','desk','dresser','night_stand','bookshelf','bathtub']
    


    #load predictions
    if pred_results==None:
        result_pkl_path='/root/userfolder/github/mmdetection/pred_results/cppf/pred_results.pkl'
    ...
    if pred_results_dir==None:
        result_dir='/root/userfolder/github/mmdetection/pred_results/cppf/'
    else:
    ...
    iou_aps,pose_aps,_,_=compute_degree_cm_mAP(pred_results, synset_names, result_dir + 'cppf_map', 
    ...
    fw = open('{0}/eval_logs.txt'.format(result_dir + 'cppf_map'), 'a')
    messages = []
    messages.append('mAP IOU:')
    for cls_id in range(1, num_classes):
        messages.append('{} 3D IoU at 10: {:.1f}'.format(synset_names[cls_id], iou_aps[cls_id, iou_thres_list.index(0.10)] * 100))
        messages.append('{} 3D IoU at 25: {:.1f}'.format(synset_names[cls_id], iou_aps[cls_id, iou_thres_list.index(0.25)] * 100))
        messages.append('{} 3D IoU at 50: {:.1f}'.format(synset_names[cls_id], iou_aps[cls_id, iou_thres_list.index(0.5)] * 100))
        messages.append('{} 3D IoU at 75: {:.1f}'.format(synset_names[cls_id], iou_aps[cls_id, iou_thres_list.index(0.75)] * 100))
        
    
    messages.append('mAP pose:')
    for cls_id in range(1, num_classes):
        for deg_thresh in degree_thres_list:
            for shift_thres in shift_thres_list:
                messages.append('{} {} degree, {}cm: {:.1f}'.format(synset_names[cls_id], deg_thresh, shift_thres, pose_aps[cls_id, degree_thres_list.index(deg_thresh), shift_thres_list.index(shift_thres)] * 100))
    
    for deg_thresh in degree_thres_list:
        for shift_thres in shift_thres_list:
            messages.append('{} degree, {}cm: {:.1f}'.format(deg_thresh, shift_thres, pose_aps[-1, degree_thres_list.index(deg_thresh), shift_thres_list.index(shift_thres)] * 100))
    
    messages.append('3D IoU at 10: {:.1f}'.format(iou_aps[-1, iou_thres_list.index(0.10)] * 100))
    messages.append('3D IoU at 25: {:.1f}'.format(iou_aps[-1, iou_thres_list.index(0.25)] * 100))
    messages.append('3D IoU at 50: {:.1f}'.format(iou_aps[-1, iou_thres_list.index(0.5)] * 100))
    messages.append('3D IoU at 75: {:.1f}'.format(iou_aps[-1, iou_thres_list.index(0.75)] * 100))

    messages.append('60 degree, 30cm: {:.1f}'.format(pose_aps[-1, degree_thres_list.index(60), shift_thres_list.index(30)] * 100))
    messages.append('40 degree, 20cm: {:.1f}'.format(pose_aps[-1, degree_thres_list.index(40), shift_thres_list.index(20)] * 100))
    messages.append('20 degree, 10cm: {:.1f}'.format(pose_aps[-1, degree_thres_list.index(20), shift_thres_list.index(10)] * 100))
    messages.append('10 degree, 10cm: {:.1f}'.format(pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(10)] * 100))
    messages.append('10 degree : {:.1f}'.format(pose_aps[-1, degree_thres_list.index(10), -1] * 100))
    messages.append('10 cm : {:.1f}'.format(pose_aps[-1, -1, shift_thres_list.index(10)] * 100))
