...
from ..functional import compute_mAP_sunrgbd,plot_mAP_sunrgbd
...
class CocoMetricSUNRGBD(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        logger.info("TEST computing metrics on NOCS now")
        ...
        for metric in self.metrics:
            ...
            if metric == 'pose':
                ...
                try: 
                    ...
                    #print(cfg.work_dir)
                    pred_results_base_dir = osp.join(cfg.work_dir,'pred_results/nocs/')
                except:
                    pred_results_base_dir='pred_results/nocs/eval'
                #print("pred_results_base_dir",pred_results_base_dir)
                #pred_results_base_dir='pred_results/nocs/spilts_xyz/'
                ...
                #comupte mAP
                evaluate_nocs(pred_results,pred_results_dir,logger)
...
def evaluate_nocs(pred_results=None,pred_results_dir=None,logger=None):
    degree_thres_list = list(range(0, 61, 1))
    shift_thres_list = [i / 2 for i in range(61)]
    iou_thres_list = [i / 100 for i in range(101)]
    #load predictions
    if pred_results==None:
        result_pkl_path='/root/userfolder/github/mmdetection/pred_results/sunrgbd/pred_results.pkl'
    ...
    shift_30_idx = shift_thres_list.index(30)
    messages = []
    messages.append('mAP:')
    ...
    messages.append('20 degree, 10cm: {:.1f}'.format(pose_aps[-1, degree_20_idx, shift_10_idx] * 100))
    messages.append('40 degree, 20cm: {:.1f}'.format(pose_aps[-1, degree_40_idx, shift_20_idx] * 100))
    messages.append('60 degree, 30cm: {:.1f}'.format(pose_aps[-1, degree_60_idx, shift_30_idx] * 100))
    ...
    for i in range(pose_aps.shape[0]):
        messages.append('mAP:')
        messages.append('{:d} 3D IoU at 10: {:.1f}'.format(i,iou_aps[i, iou_10_idx] * 100))
        ...
        messages.append('20 degree, 10cm: {:.1f}'.format(pose_aps[i, degree_20_idx, shift_10_idx] * 100))
        messages.append('40 degree, 20cm: {:.1f}'.format(pose_aps[i, degree_40_idx, shift_20_idx] * 100))
        messages.append('60 degree, 30cm: {:.1f}'.format(pose_aps[i, degree_60_idx, shift_30_idx] * 100))
    ...
    messages.append('Acc:')
    messages.append('3D IoU at 10: {:.1f}'.format(iou_acc[-1, iou_10_idx] * 100))
    ...
    messages.append('20 degree, 10cm: {:.1f}'.format(pose_acc[-1, degree_20_idx, shift_10_idx] * 100))
    messages.append('40 degree, 20cm: {:.1f}'.format(pose_acc[-1, degree_40_idx, shift_20_idx] * 100))
    messages.append('60 degree, 30cm: {:.1f}'.format(pose_acc[-1, degree_60_idx, shift_30_idx] * 100))
    ...
    # plot
    plot_mAP_sunrgbd(iou_aps, pose_aps, result_dir, iou_thres_list, degree_thres_list, shift_thres_list)