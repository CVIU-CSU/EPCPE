...
from ..layers.transformer import SAMAdapterTransformerDecoder
...
class SAMADAPTER_DABADAPTER(DETR):
    ...
    def _init_layers(self) -> None:
        ...
        self.sam_decoder = SAMAdapterTransformerDecoder(**self.sam_decoder)
    ...
    def forward_decoder(self, query: Tensor, query_pos: Tensor, query_pos_sam:Tensor, 
        ...
        if self.learned_gate:
            gate=self.sigmoid(self.hs_sum_weight)
        else:
            gate=self.hs_sum_weight

        hidden_states_sam=(gate)*hidden_states_sam+(1-gate)*hidden_states
        #print("gate",gate)
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        # # print(x[0],x[0].shape)
        # visual_img=visualizer.draw_featmap(x[0][0], channel_reduction='squeeze_mean')
        # #visualizer.show(visual_img)
        # visualizer.add_image('vit-det-sam', visualizer.get_image())
        return x
    ...
    def predict(self,
        ...
        # # print(image[0].permute(1,2,0),image[0].permute(1,2,0).shape)
        # # print(x[0],x[0].shape)
        # print(head_inputs_dict['hidden_states_sam'][0].shape)
        # visual_img=visualizer.draw_featmap(head_inputs_dict['hidden_states_sam'][0], channel_reduction='squeeze_mean',topk=5)
        # #visualizer.show(visual_img)
        # visualizer.add_image('sam adapter dab adapter', visualizer.get_image(),1)
