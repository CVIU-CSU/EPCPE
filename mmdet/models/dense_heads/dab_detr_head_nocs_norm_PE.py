...
class DABDETRHeadNOCSNormPE(ConditionalDETRHead): 
    ...
    def __init__(
            ...
            **kwargs) -> None:
        super(DABDETRHeadNOCSNormPE,self).__init__(**kwargs)
    ...
    def forward(self, hidden_states: Tensor,
        ...
        bbox_embedding = self.pe.forward_with_coords(bboxes_xyxy.reshape(-1,2,2)) #2*256

        n,b,q,d=hidden_states.shape
        hidden_states=hidden_states.reshape(-1,1,self.embed_dims) #prepare for attention #1*256

        hidden_states_bbox = self.pose_attention(q=hidden_states,k=bbox_embedding,v=bbox_embedding) 
        hidden_states_bbox = hidden_states_bbox.reshape(n,b,q,d)
        layers_bbox_preds_T = self.fc_reg_T(hidden_states_bbox)
        layers_bbox_preds_R = self.fc_reg_R(hidden_states_bbox)
