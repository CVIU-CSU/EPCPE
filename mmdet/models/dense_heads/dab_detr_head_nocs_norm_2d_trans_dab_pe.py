...
from ..layers import MLP, inverse_sigmoid, coordinate_to_encoding
...
class DABDETRHeadNOCSNorm2DTrans(ConditionalDETRHead): 
    ...
    def __init__(
            ...
            **kwargs) -> None:
        super(DABDETRHeadNOCSNorm2DTrans,self).__init__(**kwargs)
    ...
    def _init_layers(self) -> None:
        ...
        # pos sine proj
        self.pos_bbox_sine_proj = Linear(self.embed_dims, self.embed_dims)
        # reg branch
    ...
    def forward(self, hidden_states: Tensor,
        ...
        bbox_sine_embed = coordinate_to_encoding(coord_tensor=layers_bbox_preds,num_feats=self.embed_dims//4).reshape_as(hidden_states)
        pos_bbox_sine_embed = self.pos_bbox_sine_proj(bbox_sine_embed)
        ln,bs,nq,ed=hidden_states.shape
        object_hidden_states = torch.cat([hidden_states.detach(),pos_bbox_sine_embed],dim=-1).view(ln,bs,nq,ed*2)
        layers_bbox_preds_T = self.fc_reg_T(object_hidden_states)
