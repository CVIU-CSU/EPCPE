...
class DABDETRHeadPhoCal(ConditionalDETRHead):
    ...
    def __init__(
            ...
            **kwargs) -> None:
        super(DABDETRHeadPhoCal,self).__init__(**kwargs)
    ...
    def _init_layers(self) -> None:
        ...
        # rot branch
        self.fc_reg_R = MLP(self.embed_dims, self.embed_dims, 9, 3) #输出6维结果
        # trs branch
    ...
    def forward(self, hidden_states: Tensor,
                references: Tensor) -> Tuple[Tensor, Tensor ,Tensor,Tensor]:
        """"Forward function.
        ...
        return layers_cls_scores,layers_bbox_preds, layers_bbox_preds_R , layers_bbox_preds_T
    ...
    def loss_by_feat( #以下内容来自DETRHead
        ...
        losses_cls,losses_bbox, losses_iou, losses_R, losses_T = multi_apply(
        ...
        num_dec_layer = 0
        for loss_cls_i,loss_bbox_i,loss_iou_i, loss_R_i, loss_T_i in \
                zip(losses_cls[:-1], losses_bbox[:-1], losses_iou[:-1], losses_R[:-1], losses_T[:-1]):
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor, bbox_R_preds: Tensor, bbox_T_preds: Tensor,
        ...
        return loss_cls,loss_bbox, loss_iou, loss_R, loss_T
    ...
    def get_targets(self, cls_scores_list: List[Tensor],
        ...
        return (labels_list, label_weights_list,bbox_targets_list,bbox_weights_list,  bbox_R_targets_list, bbox_R_weights_list, 
                bbox_T_targets_list, bbox_T_weights_list, num_total_pos, num_total_neg)

    def _get_targets_single(self, cls_score: Tensor,bbox_pred: Tensor, rot_pred: Tensor, pos_pred: Tensor,
        ...
        pred_instances = InstanceData(scores=cls_score, bboxes=bbox_pred, rot=rot_pred , pos=pos_pred)
        ...
        gt_rot = gt_instances.rot
        gt_pos = gt_instances.pos
        gt_labels = gt_instances.labels
        ...
        pos_gt_rot = gt_rot[pos_assigned_gt_inds.long(), :]
        pos_gt_pos = gt_pos[pos_assigned_gt_inds.long(), :]
        ...
        bbox_R_targets[pos_inds] = pos_gt_rot
        bbox_T_targets[pos_inds] = pos_gt_pos
    ...
    def predict_by_feat(self,
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            results = self._predict_by_feat_single(cls_score,bbox_pred, bbox_pred_R, bbox_pred_T,
                                                   img_meta, rescale)
