...
from ..utils import PositionEmbeddingRandom,Attention


@MODELS.register_module()
class DABDETRHeadNOCSNorm6DPE(ConditionalDETRHead): 
    ...
    def __init__(
            ...
            **kwargs) -> None:
        super(DABDETRHeadNOCSNorm6DPE,self).__init__(**kwargs)
        ...
        self.pe = PositionEmbeddingRandom(self.embed_dims//2)
        self.pose_attention = Attention(self.embed_dims,8,1)
    ...
    def _init_layers(self) -> None:
        ...
        # rot branch
        self.fc_reg_R = MLP(self.embed_dims*2, self.embed_dims, 6, 3) #先输出6维结果 转成9维
        # trs branch
        self.fc_reg_T = MLP(self.embed_dims*2, self.embed_dims, 3, 3) #3X1 1 Mat
        # reg branch
    ...
    def forward(self, hidden_states: Tensor,
        ...
        layers_bbox_preds_norm = layers_bbox_preds.reshape(-1,4)
        bboxes_xyxy = bbox_cxcywh_to_xyxy(layers_bbox_preds_norm)
        bbox_embedding = self.pe.forward_with_coords(bboxes_xyxy.reshape(-1,2,2))

        hidden_states_bbox = self.pose_attention(q=bbox_embedding,k=hidden_states,v=bbox_embedding)

        layers_bbox_preds_T = self.fc_reg_T(hidden_states_bbox)
        layers_bbox_preds_R_tmp = self.fc_reg_R(hidden_states_bbox)
