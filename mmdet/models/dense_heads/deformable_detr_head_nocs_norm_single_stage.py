...
class DeformableDETRHeadNOCSNormSS(DETRHead):
    ...
    def __init__(self,
        ...
        super(DeformableDETRHeadNOCSNormSS,self).__init__(*args, **kwargs)
    ...
    def init_weights(self) -> None:
        ...
        for m in self.fc_reg_R: # follow offical init MLP
            constant_init(m.layers[-1], 0, bias=0)
        for m in self.fc_reg_T:
            constant_init(m.layers[-1], 0, bias=0)
        for m in self.fc_reg_size:
            constant_init(m.layers[-1], 0, bias=0)
        for m in self.fc_reg_scale:
            constant_init(m.layers[-1], 0, bias=0)
    ...
    def forward(self, hidden_states: Tensor,
        ...
        for layer_id in range(hidden_states.shape[0]):
            ...
            outputs_R = self.fc_reg_R[layer_id](hidden_state)
            outputs_T = self.fc_reg_T[layer_id](hidden_state)
            outputs_size = self.fc_reg_size[layer_id](hidden_state)
            outputs_scale = self.fc_reg_scale[layer_id](hidden_state)
