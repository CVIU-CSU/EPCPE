...
class SAMDABDETRHeadNOCSNorm6DROT(ConditionalDETRHead): 
    ...
    def __init__(
            self,
            loss_R:ConfigType = dict(type='ROTLoss', loss_weight=1.0),
            ...
            **kwargs) -> None:
        super(SAMDABDETRHeadNOCSNorm6DROT,self).__init__(**kwargs)
    ...
    def _init_layers(self) -> None:
        ...
        # rot branch
        self.fc_reg_R = MLP(self.embed_dims, self.embed_dims, 6, 3) #先输出9维结果
        # trs branch
    ...
    def forward(self, hidden_states: Tensor,references: Tensor
                    ,hidden_states_sam: Tensor) -> Tuple[Tensor, Tensor ,Tensor,Tensor,Tensor]:
        """"Forward function.
        ...
        layers_bbox_preds_R_tmp = self.fc_reg_R(hidden_states_sam)
        layers_bbox_preds_R = self.rotation_6d_to_matrix(layers_bbox_preds_R_tmp)
        layers_bbox_preds_T = self.fc_reg_T(hidden_states_sam)
        layers_bbox_preds_size = self.fc_reg_size(hidden_states_sam)
        layers_bbox_pred_scale = self.fc_reg_scale(hidden_states_sam)
    ...
    def predict(self,
        ...
        last_layer_hidden_state_sam = hidden_states_sam[-1].unsqueeze(0)
        outs = self(last_layer_hidden_state, last_layer_reference, last_layer_hidden_state_sam)

        predictions = self.predict_by_feat(
            *outs, batch_img_metas=batch_img_metas, rescale=rescale)
        return predictions

    def loss(self, hidden_states: Tensor, references: Tensor, hidden_states_sam: Tensor,
        ...
        outs = self(hidden_states, references, hidden_states_sam)
        loss_inputs = outs + (batch_gt_instances, batch_img_metas)
        losses = self.loss_by_feat(*loss_inputs)
        return losses

    def loss_and_predict(
            self, hidden_states: Tensor, references: Tensor, hidden_states_sam: Tensor,
    ...
    def loss_by_feat( #以下内容来自DETRHead
        ...
        losses_cls, losses_bbox, losses_iou, losses_R, losses_T, losses_size,losses_scale = multi_apply(
        ...
        num_dec_layer = 0
        for loss_cls_i,loss_bbox_i,loss_iou_i, loss_R_i, loss_T_i, loss_size_i, loss_scale_i in \
                zip(losses_cls[:-1], losses_bbox[:-1], losses_iou[:-1], losses_R[:-1], losses_T[:-1], losses_size[:-1], losses_scale[:-1]):
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor, bbox_R_preds: Tensor, bbox_T_preds: Tensor,
        ...
        return loss_cls,loss_bbox, loss_iou, loss_R, loss_T, loss_size ,loss_scale
    ...
    def _get_targets_single(self, cls_score: Tensor,bbox_pred: Tensor, rot_pred: Tensor, pos_pred: Tensor, size_pred: Tensor, scale_pred: Tensor,
        ...
        bbox_R_weights = torch.zeros(rot_pred.shape[0],1).cuda()
