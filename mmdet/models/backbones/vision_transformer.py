...
class VisionTransformer(BaseBackbone):
    ...
    def init_weights(self):
        ...
        if self.pretrained is None:
            ...
            logger.info(f'load model backbone from: {self.pretrained}')
            ## Load SAM-B pretrained weights
            assert self.pretrained in {"dino-b","dino-base","dino-small","dino-s"}
            if self.pretrained=="dino-b":
                state_dict = torch.load('pretrained/vit-base-p14_dinov2-pre_3rdparty_20230426-ba246503.pth')
            elif self.pretrained=="dino-base":
                state_dict = torch.load('pretrained/vit-base-p14_dinov2-pre_3rdparty_20230426-ba246503.pth')
            elif self.pretrained=="dino-s":
                state_dict = torch.load('pretrained/vit-small-p14_dinov2-pre_3rdparty_20230426-5641ca5a.pth')
            elif self.pretrained=="dino-small":
                state_dict = torch.load('pretrained/vit-small-p14_dinov2-pre_3rdparty_20230426-5641ca5a.pth')
    ...
    def forward(self, x):
        ...
        # print(image.shape)
        # feature_x=outs[-1][0]
        # visualizer=Visualizer(image=np.array(image[0].permute(1,2,0)),
        #         vis_backends=[dict(type='LocalVisBackend')],
        #                             save_dir='tmp')

        # print(image[0].permute(1,2,0),image[0].permute(1,2,0).shape)
        # print(feature_x,feature_x.shape)
        # visual_img=visualizer.draw_featmap(feature_x, channel_reduction='select_max')
        # visualizer.show(visual_img)
        # visualizer.add_image('dinov2_base_feature', visualizer.get_image())
