...
from .utils.data_utils import get_K,get_bbox_conner,reproj_3d_2d
import json
...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'rots' in instances:
            nocs_camera_intrinsics = [577.5, 577.5, 319.5, 239.5]
            nocs_real_intrinsics = [591.0125, 590.16775, 322.525, 244.11084]
            phocal_intrinsics = [591.0125, 590.16775, 322.525, 244.11084] #还没填入phocal的内参
            wild6d_intrinsics = [435.21649169921875,435.21649169921875,242.00372314453125,319.216796875]
            #wild6d_intrinsics = [593.9667358398438,594.9925537109375,240.4332733154297,322.6242980957031]
            sunrgbd_intrinsics = [529.5,529.5,365,265]
            objectron_intrinsics = [1576.045,1576.045,728.600,935.864]
            ...
            #rotations = instances.rots
            rotations = instances.rots
            positions = instances.poses
            labels = instances.labels
            sizes = instances.sizes
            # if 'rots_norm' in instances:
            #     rotations_norm = instances.rots_norm
            #     #first_rot_norm=np.eye(3)
            #     first_rot_norm=rotations_norm[0].reshape(3,3)
            #     for rot_norm in rotations_norm:
            #         rot_norm=rot_norm.reshape(3,3)

            #         diff_angle=np.arccos(np.clip(0.5*((rot_norm@first_rot_norm.T).trace()-1),-1,1))*180/np.pi
            #         print(diff_angle)

            #         I_mat=rot_norm@rot_norm.T
            #         print(I_mat)
            ...
            #print(rotations,positions,labels)
            for rot,pos,label,size in zip (rotations,positions,labels,sizes):
                # print(rot,pos,label)
                ...
                for line in lines:
                    x_list.append(reproj_box_2d[line[0]][0])
                    x_list.append(reproj_box_2d[line[1]][0])
                    y_list.append(reproj_box_2d[line[0]][1])
                    y_list.append(reproj_box_2d[line[1]][1])
                
                # print(x_list,y_list)
                x_list=np.array(x_list).reshape(-1,2)
                y_list=np.array(y_list).reshape(-1,2)
                self.draw_lines(x_list,y_list,colors)
    ...
    def add_datasample(
        ...
        """
        #print("this is a add data sample")
