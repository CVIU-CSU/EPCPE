...
def save_demo_image(pose_preds, K, image_path, bbox_conners, draw_box=True, save_path=None):
    """ 
    Project 3D bbox by predicted pose and visualize
    """
    #box3d = np.loadtxt(box3d_path)
    if (len(pose_preds)==len(bbox_conners)):
        print("yes")

    image_full = cv2.imread(image_path)

    if isinstance(pose_preds,list):#如果有很多pose_preds
        for pose_pred,box3d in zip(pose_preds,bbox_conners):
            if draw_box:
