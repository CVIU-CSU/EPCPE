...
model = dict(
    ...
    neck=dict(type='FPN', in_channels=[768], out_channels=256, num_outs=1),
    encoder=dict(
        num_layers=3,
        layer_cfg=dict(
            self_attn_cfg=dict(
...
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=dict(
        type='AdamW', lr=0.0001, betas=(0.9, 0.999), weight_decay=0.005),
...
launcher = 'pytorch'
work_dir = './work_dirs/dab-detr_nocs_swin_t_Norm_CRtrain_3enc_3dec_640'
